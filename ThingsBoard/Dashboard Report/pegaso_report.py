            ...
            if valores_atributos:
                for item in valores_atributos:
                    if item['key'] == 'power station':
                        ...
                        last_activity_time = item['value']
                        row_data[headers.index('lastActivityTime')] = convertir_unix_a_fecha_hora(last_activity_time)

            # Filtrar dispositivos inactivos por 15 minutos o más
            if last_activity_time:
                tiempo_actual = datetime.datetime.now().timestamp() * 1000  # Tiempo actual en milisegundos
                tiempo_inactividad = tiempo_actual - last_activity_time

                if tiempo_inactividad < 15 * 60 * 1000:  # 15 minutos en milisegundos
                    continue
